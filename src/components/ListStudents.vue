            ...
            <v-card-text>
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12">
                    <v-select label="Estado" v-model="editedItem.estado" :items="listState" item-text="state"
...
export default {
  data: () => ({
    items: [],
    listState: [
      { state: 'ACTIVO', value: 'ACTIVO' },
      { state: 'RETIRADO', value: 'RETIRADO' },
      { state: 'GRADUADO', value: 'GRADUADO' }
    ],
    headers: [
      {
        text: "Nombre",
    ...
    async updateStudent(editedItem) {
      axios
        .put(`api/students/update/${editedItem.id}`, {
          ...
          estado: editedItem.estado === 'ACTIVO' ? 1 : editedItem.estado === 'RETIRADO' ? 0 : 2,
        })
        .then(() => {
          this.getStudents();
        })
